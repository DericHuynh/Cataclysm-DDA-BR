        ...
        if *screen.get() == Screen::ExamineLook {
            if let Some((dx, dy)) = movement_delta(&event.action) {
                let current =
                    cursor
                        .tile
                        .unwrap_or(WorldPos::new(0, 0, cdda_core::coords::ZLevel::new(0)));
...
fn movement_delta(action: &GameAction) -> Option<(i32, i32)> {
    match action {
        GameAction::Move(dir) => match dir {
            cdda_input::Direction::North => Some((0, -1)),
            cdda_input::Direction::South => Some((0, 1)),
            cdda_input::Direction::West => Some((-1, 0)),
            cdda_input::Direction::East => Some((1, 0)),
            cdda_input::Direction::NorthWest => Some((-1, -1)),
            cdda_input::Direction::NorthEast => Some((1, -1)),
            cdda_input::Direction::SouthWest => Some((-1, 1)),
            cdda_input::Direction::SouthEast => Some((1, 1)),
            _ => None,
        },
        _ => None,
    }
