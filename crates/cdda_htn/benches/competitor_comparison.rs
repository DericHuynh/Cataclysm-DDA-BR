//! Like-for-like comparison of `cdda_htn` against the other HTN planner in
//! the Bevy ecosystem:
//!
...
//!   machinery (cdda_htn: one plan + full scratchpad execution; BAE:
...
//!   execute/validate their next step (cdda_htn: compiled plan cursor, the
...
//!   from scratch in one frame. It was structurally unfair: cdda_htn
...
//! cdda_htn raises its sanity limit for this domain
...
//!
//! The cdda_htn side encodes the corridor as a **recursive selector**
...
//! cdda_htn's total-order method — flatten the corridor in one O(n) pass
...
//!
//! The same corridor with the cdda_htn side re-encoded the way BAE's model
...
// ---------------------------------------------------------------------------
// cdda_htn — typed components, closure conditions/effects
// ---------------------------------------------------------------------------

mod bhtn_side {
    use cdda_htn::planner::{HtnPlanner, Plan};
    use cdda_htn::state::PlanState;
    use cdda_htn::tasks::{TaskBuilder, TaskFn};
...
fn competitor_comparison(c: &mut Criterion) {
    // --- Single-actor: one complete AI episode -----------------------------
    {
        ...
        group.bench_function("cdda_htn", |b| {
    ...
    // --- Deep chain, matched encoding: cdda_htn total-order method vs BAE
    //     Sequence ---
    {
