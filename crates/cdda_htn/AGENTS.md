...
- **Benchmarks** live in `benches/ai_throughput.rs` (Criterion). They run the planner **through a real Bevy `Schedule`** (the same path a production app uses, which initializes the `ComputeTaskPool`): 200k miner entities are spawned (each carrying a `MinerState` + `Plan` component) and a system plans each entity per frame via `Query::par_iter_mut` over the multi-threaded executor, writing the `Plan` component per entity — plus a single-actor latency case. This measures the same planner-through-query + parallel-execution path the game uses, not just raw function calls.
