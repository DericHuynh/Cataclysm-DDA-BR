...
fn named_branches_are_recorded() {
    ...
    let methods = match domain.get_task("root").unwrap() {
        cdda_htn::Task::Compound(c) => &c.methods,
...
/// value [`HighestUtility`](cdda_htn::domain::SelectionPolicy::HighestUtility)
/// ranks by.
#[test]
...
fn cost_is_recorded_on_primitives() {
    ...
    let domain = HtnDomain::from_root(root).build().unwrap();
    let cdda_htn::Task::Primitive(p) = domain.get_task("cheap").unwrap() else {
...
impl Searcher for FixedSearcher {
    fn search(&self, domain: &HtnDomain, _state: &PlanState) -> Option<Plan> {
        ...
        Some(Plan::compiled(
            vec![idx as u32],
            vec![1],
            cdda_htn::planner::PlanStatus::Complete,
            None,
        ))
    }
}
