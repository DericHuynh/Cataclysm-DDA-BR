...
fn plan_of<F: TaskFn>(domain: &cdda_htn::HtnDomain, _root: F, state: &PlanState) -> Vec<String> {
...
fn scratch_get<T>(domain: &cdda_htn::HtnDomain, state: &PlanState) -> T
where
    T: cdda_htn::state::PlanComponent + Clone,
{
    state
        .get_slot::<T>(
            domain
