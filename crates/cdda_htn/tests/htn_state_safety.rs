...
use cdda_htn::selection::LookaheadMode;
use cdda_htn::state::{PlanState, RegistryBuilder};
use cdda_htn::tasks::{GoalBuilder, GoalFn, TaskBuilder, TaskFn};
use cdda_htn::{BackPlanner, FieldSet, HtnDomain, HtnResult};
...
fn backward_plan_is_a_compiled_program() {
    ...
    let state = PlanState::build(&domain.components).set(Gold(0)).finish();
    let mut back = cdda_htn::BackPlanner::new(&domain);
