//! Pins for the Bevy ECS execution layer (`cdda_htn::ecs`): the [`HtnAgent`]
...
use cdda_htn::ecs::{htn_ai_system, AgentRoot, HtnAgent, HtnConfig, PlanEvery};
use cdda_htn::tasks::{GoalBuilder, TaskBuilder};
...
fn driver_handles_multiple_agents_and_config_tuning() {
    ...
    cfg.lookahead = cdda_htn::selection::LookaheadMode::Off;
    let mut world = World::new();
...
mod hotpatch {
    ...
    fn hot_patch_rebuilds_domain_and_drops_plans() {
        ...
        world.insert_resource(HtnConfig::with_rebuild(domain_with_value(1), || {
            Ok::<_, cdda_htn::HtnError>(domain_with_value(2))
        }));
        let entity = world.spawn((Battery(0), HtnAgent::default())).id();
    ...
    fn failed_rebuild_keeps_the_old_domain() {
        let mut world = World::new();
        world.insert_resource(HtnConfig::with_rebuild(domain_with_value(1), || {
            Err(cdda_htn::HtnError::builder("patch does not compile"))
        }));
        let entity = world.spawn((Battery(0), HtnAgent::default())).id();
    ...
    fn rebuild_with_new_component_resets_the_scratchpad() {
        use cdda_htn::state::PlanComponent;
        ...
        world.insert_resource(HtnConfig::with_rebuild(plain_domain(), || {
            Ok::<HtnDomain, cdda_htn::HtnError>(domain_with_extra())
        }));
        let entity = world
...
/// [`AgentRoot`](cdda_htn::ecs::AgentRoot) plans the registered extra root
