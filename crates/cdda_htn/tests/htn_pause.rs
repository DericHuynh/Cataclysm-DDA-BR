...
//! [`ResumePoint`](cdda_htn::planner::ResumePoint) (tasks + committed MTR),
//! and [`HtnPlanner::resume`](cdda_htn::planner::HtnPlanner::resume)
...
use cdda_htn::mcts::MctsSearcher;
use cdda_htn::planner::{HtnPlanner, PlanStatus, ResumePoint, ResumeStep};
use cdda_htn::selection::HtnSearchStrategy;
use cdda_htn::state::PlanState;
use cdda_htn::tasks::TaskBuilder;
...
fn resume_fails_with_no_plan_when_the_suffix_cannot_decompose() {
    ...
    let err = planner
        ...
        .unwrap_err();
    assert!(
        matches!(err, cdda_htn::HtnError::NoPlan),
...
fn resume_rejects_out_of_bounds_task_indices() {
    ...
    let err = planner
        ...
        .unwrap_err();
    match err {
        cdda_htn::HtnError::UnregisteredTask { type_name } => {
