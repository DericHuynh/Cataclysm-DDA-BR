...
use cdda_htn::ahtn::{Ahtn, AhtnOutcome};
use cdda_htn::state::PlanState;
use cdda_htn::tasks::{TaskBuilder, TaskFn};
use cdda_htn::{HtnDomain, HtnError, HtnResult};
...
fn ahtn_schedules_the_first_topological_order() {
    ...
    let executed = {
        ...
        for &i in &outcome.plan {
            if let cdda_htn::Task::Primitive(p) = &domain.tasks[i as usize] {
                for e in &p.effects {
                    e.apply(&mut s);
                }
            }
...
fn dungeon_setup(
    min_root: impl cdda_htn::tasks::TaskFn,
