...
impl DomainBuilder {
    ...
    pub fn build(self) -> HtnResult<HtnDomain> {
        finish_bake(self.rec, self.goals, 0)
    }
}

/// Validate and freeze a fully-recorded network into an [`HtnDomain`].
///
/// Shared by [`DomainBuilder::build`] (the function-recording frontend, whose
/// root is always the first recorded task) and
/// [`graph::GraphBuilder::build`](crate::graph::GraphBuilder::build) (the
/// explicit-handle frontend, whose root is an arbitrary reserved node) so the
/// GTN compilations, validation, bake, and summary inference cannot drift
/// between the two frontends. `root_index` is the position of the root task in
/// `rec.tasks` and must be a compound task.
pub(crate) fn finish_bake(
    ...
    let wrapped = crate::gtn::apply_sharing(&mut rec)?;
    crate::gtn::apply_insertion(&mut rec, &wrapped)?;
    ...
    rec.errors.extend(rec.registry.take_errors());
    ...
    for rref in &rec.extra_roots {
        let Some(&idx) = rec.index_of.get(rref) else {
            continue;
        };
        let (name, proto) = (&rec.tasks[idx].1, &rec.tasks[idx].2);
    ...
    let mut tasks: Vec<Task> = Vec::with_capacity(rec.tasks.len() + goals.len());
    let mut index_of: HashMap<Ustr, usize> = HashMap::with_capacity(rec.tasks.len());
    let mut type_index: HashMap<TypeId, usize> = HashMap::with_capacity(rec.tasks.len());

    for (i, (tid, name, proto)) in rec.tasks.into_iter().enumerate() {
                    ...
                    .map(|m| {
                        ...
                        Ok(Method {
                            ...
                            subtasks: m
                                ...
                                .iter()
                                .map(|(tid, _, _)| {
                                    *rec.index_of.get(tid).expect("queued task recorded")
                                        as u32
                                })
                                .collect(),
                            order,
    ...
    if !tasks[root_index].is_compound() {
        return Err(HtnError::builder(
            "the root task declared no branches — forward planning needs a compound root",
        ));
    }
    ...
    let mut domain = HtnDomain {
        ...
        index_of,
        type_index,
        components: rec.registry.freeze(),
        summaries: Vec::new(),
    };
