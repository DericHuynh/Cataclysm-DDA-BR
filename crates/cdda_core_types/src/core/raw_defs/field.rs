use crate::core::raw_defs::cdda_types::RawValue;
...
pub struct FieldDef {
    ...
    /// Whether field doubles as a splatter (data uses `is_splattering`).
    #[serde(default)]
    pub is_splatter: Option<bool>,

    /// Whether field is splattering (alternate name).
    #[serde(default)]
    pub is_splattering: Option<bool>,
    ...
    /// Description affix (e.g. "covered_in").
    #[serde(default)]
    ...
    /// Looks like another field type
    #[serde(default)]
    pub looks_like: Option<String>,

    /// Half life (string duration or number).
    #[serde(default)]
    pub half_life: Option<RawValue>,

    /// Display field (parent field type) - can be string ID or boolean.
    #[serde(default)]
    pub display_field: Option<RawValue>,

    /// Legacy enum ID.
    #[serde(default)]
    pub legacy_enum_id: Option<u32>,

    /// Whether the field has accelerated decay.
    #[serde(default)]
