...
const INVENTORY_ACTIONS: &[(&str, BindableAction)] = &[
...
const CRAFTING_ACTIONS: &[(&str, BindableAction)] = &[
    ("navigate", BindableAction::NavigateUp),
...
const DEV_SPAWN_ACTIONS: &[(&str, BindableAction)] = &[
    ("navigate", BindableAction::NavigateUp),
...
const EXAMINE_BASE_ACTIONS: &[(&str, BindableAction)] = &[
    ("drop", BindableAction::Drop),
    ("wield", BindableAction::UseItem),
];

const CHARACTER_ACTIONS: &[(&str, BindableAction)] = &[
    ("switch tab", BindableAction::NavigateNextTab),
    ("scroll", BindableAction::NavigateUp),
];

// ---------------------------------------------------------------------------
// OnEnter systems — fire instantly when screen state changes
// ---------------------------------------------------------------------------

pub fn on_enter_inventory_actions(mut ctx: ResMut<ContextActions>) {
    ctx.populate(INVENTORY_ACTIONS);
}

pub fn on_enter_crafting_actions(mut ctx: ResMut<ContextActions>) {
    ctx.populate(CRAFTING_ACTIONS);
}

pub fn on_enter_dev_spawn_actions(mut ctx: ResMut<ContextActions>) {
    ctx.populate(DEV_SPAWN_ACTIONS);
}

pub fn on_enter_examine_actions(mut ctx: ResMut<ContextActions>) {
    ctx.populate(EXAMINE_BASE_ACTIONS);
}

pub fn on_enter_character_actions(mut ctx: ResMut<ContextActions>) {
    ctx.populate(CHARACTER_ACTIONS);
}
