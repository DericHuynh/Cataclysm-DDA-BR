...
pub struct MaterialDef {
    ...
    /// Catch-all for any extra JSON fields.
    #[serde(default)]
    pub extra: Option<serde_json::Value>,

    /// Salvaged into item id.
    #[serde(default)]
    pub salvaged_into: Option<serde_json::Value>,

    /// Fuel data (raw JSON object).
    /// The data uses `"fuel_data"` as the key; this is the flexible form.
    #[serde(default)]
    pub fuel_data: Option<serde_json::Value>,

    /// Burn products (what items are produced when burning).
    #[serde(default)]
    pub burn_products: Option<serde_json::Value>,

    /// Burn data (per-intensity fuel/smoke/burn values).
    #[serde(default)]
    pub burn_data: Option<serde_json::Value>,
    ...
    #[serde(default)]
    pub bash_dmg_verb: Option<serde_json::Value>,

    /// Cut damage verb — can be a plain string like `"gouged"`
    /// or an object like `{"ctxt": "verb", "str": "dented"}`.
    #[serde(default)]
    pub cut_dmg_verb: Option<serde_json::Value>,
