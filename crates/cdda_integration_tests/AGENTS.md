...
- **Three cross-cutting tests live here today:**
  - `crafting_time_test.rs` — exercises `cdda_sim::actor::turn` AP costs, `cdda_sim::crafting::systems::{start_craft, continue_crafts}`, and the `cdda_sim::inventory` interaction. Owner in spirit: `cdda_sim::crafting`.
