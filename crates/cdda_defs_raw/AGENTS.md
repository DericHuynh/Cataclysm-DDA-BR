...
- Two top-level submodules: `src/raw_defs/` (139 files, one per def category — `htn.rs` is the newest) and `src/raw_types/` (3 files: `copy_from`, `localized`, `mod`).

## Local Contracts
- **One file per def category.** `src/raw_defs/<type>.rs` mirrors `data/core/<type>.json`. The 139 categories cover the pipeline as of this writing; new categories go in their own `raw_defs/<type>.rs` file.
...
- `cargo nextest run --workspace` exercises the full data pipeline (this crate is consumed by `cdda_data` and the 139 raw defs are used in `tests/def_world_load.rs` and `tests/hot_reload_*.rs`).
...
- `src/raw_defs/` — 139 files, one per CDDA def category. Listed in `src/raw_defs/mod.rs`; new categories are added to that module list and get their own file. `htn.rs` — `HtnCompoundDef` (`"type": "htn_compound"`: declared `parameters`, ordered `methods` with `when` predicate calls and `operator`/`task` steps, raw `args` carried as `serde_json::Value`) — the typed AST for mod-authored HTN behavior; the kernel semantics live in `cdda_sim::ai::htn`.
