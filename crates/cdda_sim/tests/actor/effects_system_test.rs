...
use cdda_components::actor::{ActiveEffects, EffectOn, Health, IsAlive, StatusEffect};
use cdda_components::sim::*;
use cdda_components::{EffectId, Time};
use cdda_sim::actor::effects::*;
...
fn decay_effect(effect: &mut StatusEffect, turns: i64) {
    effect.remaining = effect.remaining - Time::from_turns(turns);
    if effect.remaining < Time::ZERO {
        effect.remaining = Time::ZERO;
    }
}
...
fn system_apply_effect_creates_entity() {
    ...
    let has_effects = test.world().entity(creature).contains::<ActiveEffects>();
    assert!(
        has_effects,
...
fn system_apply_multiple_effects() {
    ...
    let effects = test.world().entity(creature).get::<ActiveEffects>();
    assert!(
        effects.is_some(),
