...
fn body_temperature_normal() {
    ...
    test.register::<cdda_components::actor::BodyTemperature>();

    let e = test.spawn((cdda_components::actor::BodyTemperature(36.5),));
    let temp = test
        .get::<cdda_components::actor::BodyTemperature>(e)
        .unwrap();
...
fn body_temperature_hypothermia() {
    ...
    let e = test.spawn((cdda_components::actor::BodyTemperature(30.0),));
    let temp = test
        .get::<cdda_components::actor::BodyTemperature>(e)
        .unwrap();
...
fn body_temperature_hyperthermia() {
    ...
    let e = test.spawn((cdda_components::actor::BodyTemperature(40.0),));
    let temp = test
        .get::<cdda_components::actor::BodyTemperature>(e)
        .unwrap();
...
fn wetness_dry() {
    let mut test = TestBed::new();
    test.register::<cdda_components::actor::Wetness>();

    let e = test.spawn((cdda_components::actor::Wetness(0),));
    let wet = test.get::<cdda_components::actor::Wetness>(e).unwrap();
    assert_eq!(wet.0, 0);
}
...
fn wetness_wet() {
    ...
    let e = test.spawn((cdda_components::actor::Wetness(100),));
    let wet = test.get::<cdda_components::actor::Wetness>(e).unwrap();
    assert_eq!(wet.0, 100);
}
...
fn wetness_soaked() {
    ...
    let e = test.spawn((cdda_components::actor::Wetness(500),));
    let wet = test.get::<cdda_components::actor::Wetness>(e).unwrap();
    assert_eq!(wet.0, 500);
}
...
fn temperature_and_wetness_independent() {
    ...
    let e = test.spawn((
        cdda_components::actor::BodyTemperature(36.5),
        cdda_components::actor::Wetness(100),
    ));
    assert!(test.get::<cdda_components::actor::BodyTemperature>(e).is_some());
    assert!(test.get::<cdda_components::actor::Wetness>(e).is_some());
}
...
fn preserves_temp_marker() {
    let mut test = TestBed::new();
    test.register::<cdda_components::item::PreservesTemp>();

    let e = test.spawn((cdda_components::item::PreservesTemp,));
    assert!(test
        .world()
        .entity(e)
        .contains::<cdda_components::item::PreservesTemp>());
}

#[test]
fn sealed_and_preserves_temp() {
    let mut test = TestBed::new();
    test.register::<cdda_components::item::Sealed>();
    test.register::<cdda_components::item::PreservesTemp>();

    let e = test.spawn((
        cdda_components::item::Sealed,
        cdda_components::item::PreservesTemp,
    ));
    assert!(test
        .world()
        .entity(e)
        .contains::<cdda_components::item::Sealed>());
    assert!(test
        .world()
        .entity(e)
        .contains::<cdda_components::item::PreservesTemp>());
