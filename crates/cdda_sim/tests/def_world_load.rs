...
fn build_def_world_in_world(
...
) -> (World, cdda_sim::def_world::DefinitionWorld) {
    ...
    world.register_component::<ItemFlagList>();
    ...
    world.register_component::<MonsterFlags>();
    ...
    world.register_component::<TerrainFlags>();
    ...
    world.register_component::<FurnitureFlags>();
    ...
    let def_world = build_def_world(&mut world, reg);
    (world, def_world)
}
...
fn test_item_def_gets_base_components() {
    ...
    assert!(world.get::<ItemFlagList>(entity).unwrap().0.is_empty());
...
fn test_item_with_flags() {
    ...
    assert_eq!(
        world.get::<ItemFlagList>(entity).unwrap().0,
        vec!["FIRE", "WET"]
    );
}
...
fn test_monster_def_gets_correct_components() {
    ...
    assert_eq!(world.get::<MonsterName>(entity).unwrap().0, "Test Monster");
...
fn test_terrain_def_gets_correct_components() {
    ...
    assert_eq!(world.get::<TerrainName>(entity).unwrap().0, "t_test");
...
fn test_furniture_def_gets_correct_components() {
    ...
    assert_eq!(world.get::<FurnitureName>(entity).unwrap().0, "Test Furn");
}
...
fn test_flags_to_vec_single() {
    assert_eq!(
        cdda_sim::def_world::flags_to_vec(&StringOrArray::Single("FLAG_A".to_string())),
...
fn test_flags_to_vec_empty() {
    assert_eq!(
        cdda_sim::def_world::flags_to_vec(&StringOrArray::Single(String::new())),
        Vec::<String>::new()
    );
}
...
fn test_flags_to_vec_multi() {
    assert_eq!(
        cdda_sim::def_world::flags_to_vec(&StringOrArray::Multi(vec![
            "FLAG_A".to_string(),
            "FLAG_B".to_string()
        ])),
...
fn test_integration_load_core_data_builds_def_world() {
    ...
    for id in &["f_chair", "f_table", "f_bed", "f_counter"] {
        ...
        assert!(
            world.get::<FurnitureName>(entity).is_some(),
            "'{}' should have FurnitureName",
            id
        );
    }
}
