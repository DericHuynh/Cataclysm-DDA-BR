...
12. **The simulation tick is deterministic and single-threaded.** `cdda_sim::tick()`
    ...
    `IntoSystem::into_system().run(&mut world)`. Bevy's parallel scheduler is never
    used for simulation. No systems touch `Res<Time>`, `std::time`, or unseeded
    randomness. This makes replays, crash reproduction, and potential future
    network synchronization tractable.
...
no Bevy `Schedule` involved. The order is fixed and documented.

```rust
// cdda_sim/src/tick.rs
pub fn tick(world: &mut World, rng: &mut Rng) {
    // Systems run in this exact order, every tick. No parallelism.
    turn_order::system.into_system().run(world);
    ai::system.into_system().run(world);
    movement::system.into_system().run(world);
    combat::system.into_system().run(world);
    crafting::system.into_system().run(world);
    needs::system.into_system().run(world);
    effects::system.into_system().run(world);
    vehicles::system.into_system().run(world);
    spawning::system.into_system().run(world);
...
| Long-range pathfinding | `bevy_northstar` | Full Bevy | HPA*; lives outside `cdda_map` |
...
| Tile rendering | `bevy_fast_tilemap` | Full Bevy | GPU-side buffers; "hundreds of fps" |
