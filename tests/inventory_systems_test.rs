//! Tests for `cdda_inventory::systems` — inventory lifecycle,
//! invlet management, stack merging, container operations, and item movement.

use bevy_ecs::prelude::*;

use cdda_components::core::coords::{WorldPos, ZLevel};
use cdda_components::core::units::Volume;
...
fn eff_pos_direct() {
    let mut t = TestBed::new();
    setup(&mut t);
    let pos = WorldPos::new(3, 4, ZLevel::new(0));
...
fn eff_pos_nested() {
    let mut t = TestBed::new();
    setup(&mut t);
    let pos = WorldPos::new(1, 2, ZLevel::new(0));
...
fn drop_removes_container_relationship() {
    let mut t = TestBed::new();
    setup(&mut t);
    let pos = WorldPos::new(5, 10, ZLevel::new(0));
    let creature = t.spawn((Inventory::default(),));
